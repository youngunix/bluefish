...
props_init_main(GHashTable * config_rc)
{
	...
	init_prop_integer(&config_rc, &main_v->props.editor_replace_unicode_quotes, "editor_replace_unicode_quotes:", 0, TRUE);
	init_prop_integer(&config_rc, &main_v->props.editor_spacingtoclick, "editor_spacingtoclick:", 0, TRUE);
	...
	init_prop_integer(&config_rc, &main_v->props.restore_search_scope, "restore_search_scope:", 0, TRUE);
...
*/	init_prop_integer(&config_rc, &main_v->props.block_folding_mode, "block_folding_mode:", 1, TRUE);
	...
	init_prop_integer(&config_rc, &main_v->props.autocomp_min_prefix_len, "autocomp_min_prefix_len:", 1, TRUE);
...
GList *update_filters(GList *current, gboolean overwrite)
{
	...
	defaults =
		g_list_append(defaults,
					  array_from_arglist(_("Decompress CSS"), "|python "PKGDATADIR"/css_decompressor|", NULL));
	defaults =
		g_list_append(defaults,
					  array_from_arglist(_("Compress CSS"), "|python "PKGDATADIR"/cssmin.py|", NULL));
#else
	defaults =
		g_list_append(defaults,
	...
	defaults =
		g_list_append(defaults,
					  array_from_arglist(_("Decompress CSS"), "|"PKGDATADIR"/css_decompressor|", NULL));
	defaults =
		g_list_append(defaults,
...
return_colorprofile_configlist(GHashTable *config_rc,  gboolean init_values)
{
	...
	init_prop_string(&config_rc, &main_v->props.btv_color_str[BTV_COLOR_MARGIN_FG], "margin_fg:", init_values ?"#000000":NULL);
	init_prop_string(&config_rc, &main_v->props.btv_color_str[BTV_COLOR_MARGIN_BG], "margin_bg:", init_values ?"#dddddd":NULL);
...
return_session_configlist(GHashTable * configlist, Tsessionvars * session)
{
	...
	/*	no longer stored by default, see https://sourceforge.net/p/bluefish/tickets/26/ */
	if (main_v->props.restore_search_scope) {
	...
	init_prop_string_with_escape(&configlist, &session->snr3_basedir, "snr_basedir:", NULL);
