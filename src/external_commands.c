...
externalp_unref(Texternalp * ep)
{
	...
	if (ep->refcount <= 0) {
		...
		if (ep->buffer_out)
			g_free(ep->buffer_out);
		if (ep->securedir) {
			g_rmdir(ep->securedir);
			g_free(ep->securedir);
		}
		if (ep->channel_in)
...
static gboolean
start_command_write_lcb(GIOChannel * channel, GIOCondition condition, gpointer data)
{
	Texternalp *ep = data;
	GError *gerror = NULL;
...
static void
child_watch_lcb(GPid pid, gint status, gpointer data)
{
	Texternalp *ep = data;
	GError *gerror = NULL;
...
static void
start_command_backend(Texternalp * ep)
{
#ifdef USEBINSH
	gchar *argv[4];
#else
	...
	g_spawn_async_with_pipes(NULL, argv, NULL, G_SPAWN_DO_NOT_REAP_CHILD | G_SPAWN_SEARCH_PATH,
							 (ep->include_stderr) ? spawn_setup_lcb : NULL, ep, &ep->child_pid,
							 (ep->pipe_in) ? &ep->standard_input : NULL,
							 (ep->pipe_out) ? &ep->standard_output : NULL, NULL, &error);
...
gboolean
operatable_on_selection(const gchar * formatstring)
{
...
create_commandstring(Texternalp * ep, const gchar * formatstr, gboolean discard_output, Tcommandparsererror *cperror)
{
	...
	*cperror=cpe_no_error;
	...
	if (!need_filename) {		/* local already implies we need a filename */
		need_filename = (need_preview_uri || strstr(formatstring, "%n") != NULL
						 || strstr(formatstring, "%u") != NULL);
	}
	...
	is_local_non_modified = (ep->bfwin->current_document->uri
							 && !ep->bfwin->current_document->modified && localname != NULL);

	DEBUG_MSG("create_commandstring,is_local_non_modified=%d, uri=%p, modified=%d\n", is_local_non_modified,
			  ep->bfwin->current_document->uri, ep->bfwin->current_document->modified);
	...
	if (need_filename) {
		table[cur].my_int = 'n';
		table[cur].my_char =
			need_local ? g_strdup(localfilename) : g_file_get_basename(ep->bfwin->current_document->uri);
		cur++;
		table[cur].my_int = 'u';
		...
		if (need_preview_uri) {
			...
			if (ep->bfwin->session->documentroot && ep->bfwin->session->webroot
				...
				&& strncmp(curi, ep->bfwin->session->documentroot,
						   strlen(ep->bfwin->session->documentroot)) == 0) {
				table[cur].my_char =
					g_strconcat(ep->bfwin->session->webroot, &curi[strlen(ep->bfwin->session->documentroot)],
								NULL);
			} else {
...
static gboolean
outputbox_io_watch_lcb(GIOChannel * channel, GIOCondition condition, gpointer data)
{
	Texternalp *ep = data;
...
static gboolean
full_data_io_watch_lcb(GIOChannel * channel, GIOCondition condition, gpointer data)
{
	Texternalp *ep = data;
...
filter_statuscode_lcb(gint status, Tbfwin * bfwin, gpointer data)
{
	...
	if (status != 0) {
		gchar *tmp =
			g_markup_printf_escaped(_("The command %s exited with error code %d. %s"), ep->commandstring,
									status,
									(status ==
									 32512) ? _("Probably this application is not installed on your system.")
									: "");
		message_dialog_new(BFWIN(ep->bfwin)->main_window, GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE,
						   _("Command returned error code"), tmp);
		g_free(tmp);
	}
#endif
}
...
outputbox_statuscode_lcb(gint status, Tbfwin * bfwin, gpointer data)
{
	Texternalp *ep = data;
	gchar *tmp =
		g_strdup_printf(_("The command %s exited with error code %d. %s"), ep->commandstring, status, "");
...
void
outputbox_command(Tbfwin * bfwin, const gchar * formatstring)
{
	command_backend(bfwin, 0, -1, formatstring, outputbox_io_watch_lcb, TRUE, outputbox_statuscode_lcb, NULL,
					NULL);
}
...
void
filter_command(Tbfwin * bfwin, const gchar * formatstring, gint begin, gint end)
{
	command_backend(bfwin, begin, end, formatstring, full_data_io_watch_lcb, FALSE, filter_statuscode_lcb,
