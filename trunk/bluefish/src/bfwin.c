...
bfwin_gotoline_search_bar_close(Tbfwin *bfwin, gboolean clean_entry_only)
{
	if (!clean_entry_only) {
		...
		if (bfwin->simplesearch_snr3run) {
			...
			simple_search_cancel_free(bfwin->simplesearch_snr3run);
...
*/	if (!main_v->props.leave_to_window_manager) {
	...
	/* finally the statusbar */
	{
		...
		/* I hope the 'x' is an average width character, the characters are usually not monospaced so these values are just averages that look good on most translations too */
